...
Your code:
```{r,echo=TRUE}
model1<-glm(data=Train_COVID,SURVIVED_bin ~ AGE_TRUNK+PATIENT_TYPE+FEMALE,family = "binomial")
logit_P = predict(model1,Test_COVID,type="response")
logit_Pred <- ifelse(logit_P > 0.5,1,0)
conf_matrix= table(Test_COVID$SURVIVED_bin, logit_Pred)
...
~~~
I will choose age, sex, and patient type as my metric, since they are statistically significant.
...
~~~
At first, I choose some health factors to work on, but the coefficient is so small so I dropped them and chooses Age, PAtient, and Female as my outcome.
...
~~~
I think Patient type, age  are most important.
...
new_data <- data.frame(mpg01 = Auto$mpg01, Auto[, -which(names(Auto) == "mpg01")])
```
...
```{r,echo=TRUE}
ggplot(new_data,aes(cylinders,mpg))+geom_boxplot(aes(group=cylinders))
ggplot(new_data,aes(horsepower,mpg))+geom_point(aes(color="Pink"))
```
~~~
Plots are above.
...
```{r,echo=TRUE}
set.seed(679)
index <- createDataPartition(new_data$mpg01, p = 0.7, list = FALSE)
...
~~~
Traning and test data are above.
...
Your code:
```{r,echo=TRUE}
lda_model <- lda(mpg01 ~ cylinders+horsepower, data = train_data)
predictions <- predict(lda_model, newdata = test_data)

conf_matrix <- table(predictions$class, test_data$mpg01)
...
Your code:
```{r,echo=TRUE}
qda_model <- qda(mpg01 ~ cylinders+horsepower, data = train_data)
predictions <- predict(qda_model, newdata = test_data)
conf_matrix1 <- table(predictions$class, test_data$mpg01)
test_error1 <- 1 - sum(diag(conf_matrix1)) / sum(conf_matrix1)
...
Your code:
```{r,echo=TRUE}
logistic_model <- glm(mpg01 ~ cylinders+horsepower, family = binomial(link = "logit"), data = train_data)
probabilities <- predict(logistic_model, newdata = test_data, type = "response")
predicted_classes <- ifelse(probabilities > 0.5, 1, 0)
conf_matrix <- table(predicted_classes, test_data$mpg01)
test_error <- 1 - sum(diag(conf_matrix)) / sum(conf_matrix)
cat("Test Error:", test_error, "\n")
```
...
library(class)
formula <- as.formula("mpg01 ~ cylinders + horsepower")
selected_data <- train_data[, c("cylinders", "horsepower")]
k_values <- c(1, 3, 5, 7, 9)
test_errors <- numeric(length(k_values))
for (i in seq_along(k_values)) {
  predictions <- knn(train = selected_data, test = test_data[, c("cylinders", "horsepower")],
                      cl = train_data$mpg01, k = k_values[i])
    conf_matrix <- table(predictions, test_data$mpg01)
  test_errors[i] <- 1 - sum(diag(conf_matrix)) / sum(conf_matrix)
}
cat("Test Errors for different values of K:\n")
...
~~~
Smallest test error is 0.1120690 given a k value equals 9 in a range of (1,3,5,7,9) 
